...
parser.addArgument(["--delay"], {
  help:
    "How long to delay before sending message, useful for generating latency",
  type: "int",
  defaultValue: 0,
  required: false,
  dest: "delay",
...
parser.addArgument(["--failure-percentage"], {
...
const forcedToFail = () => Math.random() * 100 < args.failurePercentage;
...
app.post("/games", (req, res) => {
  ...
  gameLobby.createGame(req.body, ({ body, status }) => {
    respond(() => {
...
app.post("/games/:gameId/moves", (req, res) => {
  ...
  gameLobby.applyMove(R.merge(req.params, req.body), ({ body, status }) => {
    respond(() => {
